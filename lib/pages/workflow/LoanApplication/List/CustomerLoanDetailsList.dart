...
import 'package:eco_los/pages/workflow/Collateral/CollateralList.dart';
import 'package:eco_los/pages/workflow/DocumentCollector/DocumetCollector.dart';
import 'package:eco_los/pages/workflow/GRT/GRTServices.dart';
import 'package:eco_los/pages/workflow/Guarantor/GuarantorDetails.dart';
...
import 'package:eco_los/pages/workflow/LoanApplication/LoanStepper.dart';
...
import 'package:eco_los/Utilities/app_constant.dart' as constant;
        ...
        if (widget.routeType == "Loan Application") {
          ...
          if (cgt1ToCgt2Gap == null) {
            ...
            globals.Dialog.alertPopup(
                context,
                "You Do not have Access for this",
                "Sync it to get further ",
                "LoginPage");
          ...
          if (cgt2ToGrtGap == null) {
            ...
            globals.Dialog.alertPopup(
                context,
                "You Do not have Access for this",
                "Please Route/Sync for Approval",
                "LoginPage");
                            ...
                            Row(
                              children: <Widget>[
                                new OutlineButton(
                                    ...
                                    textColor: Colors.blue,
                                    onPressed: () {
                                      _AddDocuments( items[index].mleadsid,
                                          items[index].mrefno,
                                          items[index].trefno,
                                          items[index].mcustname,
                                          items[index].mcustmrefno,
                                          items[index].mcusttrefno,
                                          items[index].mcustno
                    ...
                    new ButtonTheme.bar(
                      ...
                      child: new ButtonBar(
                        children: <Widget>[
                          ...
                          /*   widget.routeType == "Loan List for CGT1" && isSyncedIndex ? new IconButton(
                            icon: new Icon(Icons.sync, color: Colors.black),
                            onPressed: () => null,
                          ):Container()
*/
  ...
  void _AddGuarantor(String mleadsid, int mrefno, int trefno) {
    ...
    Navigator.push(
      ...
      new MaterialPageRoute(
          builder: (context) => new GuarantorDetails(mleadsid, mrefno,
              trefno)), //When Authorized Navigate to the next screen
    );
  }

  void _AddCollateral(String mleadsid, int mrefno, int trefno) {
    print("_AddCollateral");
    Navigator.push(
      context,
      new MaterialPageRoute(
          builder: (context) => new CollateralList(mleadsid, mrefno,
              trefno)), //When Authorized Navigate to the next screen
    );
  }

  void _AddDocuments(String mleadsid, int mrefno, int trefno, String custname, int cusmref, int custref, int cusno) {
    print("DocumetCollector");
    Navigator.push(
      context,
      new MaterialPageRoute(
          builder: (context) => new DocumetCollector(mleadsid, mrefno,
              trefno,custname,cusmref,custref,cusno)), //When Authorized Navigate to the next screen
    );
  }
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: ListBody(
              children: <Widget>[Text('Are you sure you want Sync this Loan')],
            ),
          ),
          actions: <Widget>[
            FlatButton(
            ...
            content: SingleChildScrollView(
              child: ListBody(
                children: <Widget>[
                  ...
                  Text('CustNo : ${custChkObj.mcustno} '),
                  Text('Leads Id : ${custChkObj.mleadsid} '),
                  Text('Error Message : ${custChkObj.merrormessage} '),
                ],
              ),
            ),
            actions: <Widget>[
    ...
    if (addWorkFLowList != null) {
      if (addWorkFLowList.cgt1 != null && addWorkFLowList.cgt1.length > 0) {
        ...
        for (int items = 0; items < addWorkFLowList.cgt1.length; items++) {
          ...
          TimelineModel model = TimelineModel(
            generaateTimemline(addWorkFLowList.cgt1[items], 1),
            position: TimelineItemPosition.right,
            /*   iconBackground: Colors.green,
                  icon: Icon(
                    Icons.blur_circular,
      ...
      if (addWorkFLowList.cgt2 != null && addWorkFLowList.cgt2.length > 0) {
        for (int items = 0; items < addWorkFLowList.cgt2.length; items++) {
          ...
          TimelineModel model = TimelineModel(
            generaateTimemline(addWorkFLowList.cgt2[items], 2),
            position: TimelineItemPosition.right,
            /*  iconBackground: Colors.green,
                  icon: Icon(
                    Icons.blur_circular,
      ...
      if (addWorkFLowList.grt != null && addWorkFLowList.grt.length > 0) {
        for (int items = 0; items < addWorkFLowList.grt.length; items++) {
          ...
          TimelineModel model = TimelineModel(
            generaateTimemline(addWorkFLowList.grt[items], 3),
            position: TimelineItemPosition.right,
            /*  iconBackground: Colors.green,
                  icon: Icon(
                    Icons.blur_circular,
      ...
      if (!(leadStatus == 7)) {
        TimelineModel model = TimelineModel(
            Text("Pending with : " + loanRotedPending),
            position: TimelineItemPosition.right,
        ...
        TimelineModel model = TimelineModel(
          Text("Completed With approval Process"),
          position: TimelineItemPosition.right,
          /*  iconBackground: Colors.green,
                icon: Icon(
                  Icons.blur_circular,
